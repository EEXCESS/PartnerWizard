...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:edm="http://www.europeana.eu/schemas/edm/" xmlns:wgs84="http://www.w3.org/2003/01/geo/wgs84-pos/"
	xmlns:foaf="http://xmlns.com/foaf/0.1/" xmlns:ebucore="http://www.ebu.ch/metadata/ontologies/ebucore#"
	xmlns:ore="http://www.openarchives.org/ore/terms/" xmlns:eexcess="http://eexcess.eu/schema/"
	...
	xmlns:ma="http://www.w3.org/ns/ma-ont#" xmlns:xsd="http://www.w3.org/2001/XMLSchema#"
		...
		<xsl:element name="rdf:RDF">
			...
			<xsl:for-each select="response/result/doc">
				...
				<xsl:element name="ore:Aggregation">
					<xsl:attribute name="rdf:about">
                        <xsl:value-of
						select="concat($mainURI,'/aggregation/')" />
                    </xsl:attribute>
				...
				<xsl:element name="eexcess:Proxy">
					...
					<xsl:call-template name="Main.PlaceTimeEvent" />
				</xsl:element>
			</xsl:for-each>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template name="Main.URI">
		<apply-templates xmlns="http://www.w3.org/1999/XSL/Transform"
			mode="m3" select="eexcessURI" />
	</xsl:template>
	<xsl:template name="Main.collectionName">
		<apply-templates xmlns="http://www.w3.org/1999/XSL/Transform"
			mode="m7" select="str[@name='sammlung']" />
	</xsl:template>
	<xsl:template name="Main.Subject">
		<apply-templates xmlns="http://www.w3.org/1999/XSL/Transform"
			mode="m4" select="str[@name='klassifikation_sachgruppe']/str" />
	</xsl:template>
	<xsl:template name="Main.Country">
		<apply-templates xmlns="http://www.w3.org/1999/XSL/Transform"
			mode="m6" select="ort_s_herstellung_EEXCESS" />
	</xsl:template>
	<xsl:template name="Main.Latitude">
		<apply-templates xmlns="http://www.w3.org/1999/XSL/Transform"
			mode="m11" select="double[@name='koordinaten_wgs84_0_wgs84_koordinate']" />
	</xsl:template>
	<xsl:template name="Main.Longitude">
		<apply-templates xmlns="http://www.w3.org/1999/XSL/Transform"
			mode="m12" select="double[@name='koordinaten_wgs84_1_wgs84_koordinate']" />
	</xsl:template>

	<xsl:template name="Main.PlaceTimeEvent">
		<apply-templates xmlns="http://www.w3.org/1999/XSL/Transform"
			mode="x2" select="double[@name='koordinaten_wgs84_0_wgs84_koordinate']" />
	</xsl:template>
	<template xmlns="http://www.w3.org/1999/XSL/Transform"
		match="double[@name='koordinaten_wgs84_0_wgs84_koordinate']" mode="x2">
			...
			<xsl:element name="edm:Event">
				...
				<xsl:element name="edm:happendAt">
					<xsl:element name="edm:Place">
						...
						<element name="wgs84:lat"><xsl:value-of select="." /></element>
				...
				<xsl:if test="../date[@name='datum_fund']">
					<xsl:element name="edm:occurredAt">
						<xsl:element name="edm:TimeSpan">
							<xsl:attribute name="rdf:about">
                        		<xsl:value-of select="concat('timespan/',generate-id())" />
                     		</xsl:attribute>

							<element name="edm:begin">
								<xsl:value-of
									select="substring-before(../date[@name='datum_fund'],'T')" />
							</element>
	...
	<template xmlns="http://www.w3.org/1999/XSL/Transform" match="eexcessURI"
		mode="m0">
		<element name="uri">
	...
	<template xmlns="http://www.w3.org/1999/XSL/Transform" match="str[@name='beschreibung']"
		mode="m1">
		<element name="dc:description">
			<call-template name="StringToString" />
		</element>
	</template>
	<template xmlns="http://www.w3.org/1999/XSL/Transform" match="str[@name='_display_']"
		mode="m2">
	...
	<template xmlns="http://www.w3.org/1999/XSL/Transform" match="ort_s_herstellung_EEXCESS"
		mode="m6">
	...
	<template xmlns="http://www.w3.org/1999/XSL/Transform" match="str[@name='uuid']"
		mode="m9">
	...
	<template xmlns="http://www.w3.org/1999/XSL/Transform"
		match="double[@name='koordinaten_wgs84_0_wgs84_koordinate']" mode="m11">
		<element name="wgs84:lat">
			<call-template name="StringToString" />
		</element>
	</template>
	<template xmlns="http://www.w3.org/1999/XSL/Transform"
		match="double[@name='koordinaten_wgs84_1_wgs84_koordinate']" mode="m12">
		<element name="wgs84:long">
			<call-template name="StringToString" />
		</element>
	</template>
