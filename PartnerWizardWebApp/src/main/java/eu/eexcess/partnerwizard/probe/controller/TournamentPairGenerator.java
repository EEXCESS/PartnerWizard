...
public class TournamentPairGenerator<T> extends PairGenerator<T>{
	...
	public TournamentPairGenerator( Collection<T> elements ){
		...
		input = new ArrayDeque<T>( elements );
		output = new ArrayDeque<T>( elements.size() );
		backup = new ArrayList<T>( elements );
	}
	...
	public Pair<T> nextPair(){
		if( input.size()>1 ){
			...
			pair = new Pair<T>( input.poll(), input.poll() );
		}
		else{
			pair = null;
		}
	...
	public List<T> getWinners( int numberOfElements ){
		...
		List<T> winners = new ArrayList<T>( Math.min( numberOfElements, output.size()+1 ) );
